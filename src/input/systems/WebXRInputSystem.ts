...
import { XRSession, XRWebGLLayerObject, XRFrame, XRReferenceSpace, XRInputSource } from "../types/WebXR"
    ...
    if (sessions.added)
      for (const entity of sessions.added) {
        ...
        if (isImmersive /*entity.name == "vr-session"*/) {
          webXRRenderer.context.makeXRCompatible().then(() =>
            session.updateRenderState({
              baseLayer: new XRWebGLLayerObject(session, webXRRenderer.context)
            })
          )
